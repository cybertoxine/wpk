        ...
        <div class="row">
          <div class="col-md-10 post-content-wrapper">
            <div class="float-right post-controls">
              ...
              <a href="{% if post.is_topic_head %}{% url 'forum_conversation:topic_update' forum.slug forum.pk topic.slug topic.pk %}{% else %}{% url 'forum_conversation:post_update' forum.slug forum.pk topic.slug topic.pk post.pk %}{% endif %}" class="btn btn-warning btn-sm" title="{% trans "Edycja" %}"><i class="fas fa-edit"></i>&nbsp;{% trans "Edycja" %}</a>
              {% endif %}
              {% get_permission 'can_delete_post' post request.user as user_can_delete_post %}
              {% if user_can_delete_post %}
              <a href="{% url 'forum_conversation:post_delete' forum.slug forum.pk topic.slug topic.pk post.pk %}" class="btn btn-danger btn-sm" title="{% trans "Usuń" %}"><i class="fas fa-times"></i></a>
              {% endif %}
            </div>
              {% spaceless %}
              ...
              <p>
                ...
                {% blocktrans trimmed with poster_url=poster_url username=post.poster|forum_member_display_name creation_date=post.created %}
                  Przez: <a href="{{ poster_url }}">{{ username }}</a> on {{ creation_date }}
                {% endblocktrans %}
                {% else %}
                {% blocktrans trimmed with poster_username=post.username creation_date=post.created %}
                  Przez: {{ poster_username }} on {{ creation_date }}
                {% endblocktrans %}
                {% endif %}
                {% endspaceless %}
                </small>
              ...
              <div class="mt-4 edit-info">
                <small class="text-muted">
                  <i class="fas fa-edit"></i>&nbsp;{% if post.updated_by %}{% trans "Ostatnio edytowane przez:" %}&nbsp;<a href="{% url 'forum_member:profile' post.updated_by_id %}">{{ post.updated_by|forum_member_display_name }}</a>&nbsp;{% else %}{% trans "Edytowany" %}&nbsp;{% endif %}{% trans "o" %}&nbsp;{{ post.updated }}, {% blocktrans count counter=post.updates_count %}edytowany {{counter }} raz.{% plural %}editowany {{counter }} razy.{% endblocktrans %}
                </small>
                {% if post.update_reason %}
                <br />
                <small class="text-muted">
                  <b>{% trans "Powód:" %}</b>&nbsp;{{ post.update_reason }}
                </small>
                {% endif %}
              </div>
              {% endif %}
          ...
          <div class="col-md-2 d-none d-md-block post-sidebar">
            ...
            <div class="posts-count text-muted"><b>{% trans "Posty:" %}</b>&nbsp;{{ post.poster.forum_profile.posts_count }}</div>
           <div><button style="border:5px solid salmon; background-color:salmon;color:white;border-radius:5px;" onclick="send_pw('{{ post.poster|forum_member_display_name }}','{{ post.subject }}')">PW</button></div>
            {% else %}
            <div class="username"><b>{{ post.username }}</b></div>
            <div class="username text-muted">{% trans "Anonimowy użytkownik" %}</div>
            {% endif %}
          </div>
        </div>
      </div>
