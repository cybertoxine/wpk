...
{% block sub_title %}{% trans "Kolejka moderacji" %}{% endblock sub_title %}

{% block content %}
<div class="row">
  <div class="col-12">
    <h1>{% trans "Kolejka moderacji" %}</h1>
  </div>
</div>
<div class="alert alert-warning" role="alert">
  <div class="float-right">
    <a href="{% url 'forum_moderation:disapprove_queued_post' post.pk %}" class="btn btn-warning">{% trans "Odrzuć" %}</a>
    <a href="{% url 'forum_moderation:approve_queued_post' post.pk %}" class="btn btn-success"><b>{% trans "Zatwierdź" %}</b></a>
  </div>
  <h4>{{ post.subject }}</h4>
  <p class="text-muted">{% if post.is_topic_head %}{% trans "Nowy temat" %}{% else %}{% trans "Nowy post" %}{% endif %}</p>
</div>
<div class="my-3 row">
  <div class="col-12">
        ...
        <div class="row">
          <div class="col-md-12 poll-content-wrapper">
            ...
            <div class="max-options-indication">
              ...
              {% blocktrans trimmed %}
              <b>1</b> odpowiedź może być wybrana
              {% endblocktrans %}
              {% else %}
              {% blocktrans trimmed with max_options=poll.max_options %}
              Maksymalnie <b>{{ max_options }}</b> odpowiedzi może być wybranych
              {% endblocktrans %}
              {% endif %}
            </div>
  ...
  <div class="col-12">
    <div class="card topic-review">
      <div class="card-header">
        <h3 class="m-0 h5 card-title">{% trans "Podgląd tematu" %}&nbsp;&ndash;&nbsp;{{ post.topic.subject }}</h3>
      </div>
      <div class="card-body">
