    ...
    <div class="card topiclist">
      <div class="p-0 card-header">
        <div class="row m-0 px-3 py-2">
          <div class="pl-0 col-md-8 col-sm-9 col-12 topic-name-col"><h3 class="m-0 card-title h5 text-dark"><i class="fas fa-comment"></i> {% trans topic_list_title %}</h3></div>
          <div class="col-md-1 d-none d-md-block text-center text-nowrap topic-count-col">{% trans "Odpowiedzi" %}</div>
          <div class="col-md-1 d-none d-md-block text-center text-nowrap topic-count-col">{% trans "Wyświetlenia" %}</div>
          <div class="pr-0 col-md-2 col-sm-3 d-none d-sm-block text-nowrap topic-last-post-col">{% trans "Ostatni post" %}</div>
        </div>
      </div>
          ...
          <table class="topic-data-table">
            <tr>
              ...
              <td class="align-top topic-name">
                <a href="{% url 'forum_conversation:topic' topic.forum.slug topic.forum.pk topic.slug topic.pk %}" class="topic-name-link">{{ topic.subject }}</a>{% if topic.is_locked %}&nbsp;<i class="fas fa-times-circle locked-indicator" title="{% trans 'Ten temat jest zamknięty' %}"></i>{% endif %}
                <div>
      ...
      <div class="row m-0 px-3 py-2">
        ...
        {% if empty_message %}
          {{ empty_message }}
        {% else %}
          {% trans "W tym forum nie ma żadnego tematu." %}
        {% endif %}
        </div>
      </div>
      {% endfor %}
