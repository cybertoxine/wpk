...
{% block sub_title %}{% if poster == request.user %}{% trans "Moje wiadomości" %}{% else %}{% blocktrans with username=poster|forum_member_display_name %}{{ username }}napisał.{% endblocktrans %}{% endif %}{% endblock sub_title %}

{% block content %}
<div class="row">
  <div class="col-12">
    <h1>{% if poster == request.user %}{% trans "Moje wiadomości" %}{% else %}{% blocktrans with username=poster|forum_member_display_name %}{{ username }}napisał.{% endblocktrans %}{% endif %}</h1>
  </div>
</div>
<div class="row">
  <div class="col-6 col-md-4 forum-actions-block">
    ...
    <p class="mt-1 mb-0 text-muted">
      ...
      znaleziono {{ counter }} post
      {% plural %}
      znaleziono {{ counter }} postów
      {% endblocktrans %}
    </p>
    {% else %}
    <p class="mt-1 mb-0 text-muted">{% trans "Nie znaleziono postów." %}</p>
    {% endif %}
  </div>
