...
{% block sub_title %}{% trans "Edytuj profil" %}{% endblock sub_title %}
  ...
  <div class="col-12">
    <div class="card">
      <div class="card-header">
        <h3 class="m-0 h5 card-title">{% trans "Edytuj profil" %}</h3>
      </div>
      <div class="card-body">
          ...
          <div class="row">
            <div class="col-md-12">
              <div id="div_id_{{ form.avatar.html_name }}" class="form-group{% if form.avatar.errors %} has-error{% endif %}">
                <div class="controls">
                  {% if form.avatar.label %}<label class="control-label" for="{{ field.auto_id }}">{{ form.avatar.label }}{% if not form.avatar.field.required %} <span class="text-muted">{% trans "(opcjonalne)" %}</span>{% endif %}</label>{% endif %}
                  {{ form.avatar }}
