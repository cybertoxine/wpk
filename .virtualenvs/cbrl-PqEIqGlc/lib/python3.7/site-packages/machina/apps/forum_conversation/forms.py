...
class PostForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['subject'].widget.attrs['placeholder'] = _('Wpisz tytuł')
        self.fields['content'].label = _('Wiadomość')
        self.fields['content'].widget.attrs['placeholder'] = _('Wpisz wiadomość')
        ...
        if (
            ...
            self.fields['lock_topic'] = forms.BooleanField(
                label=_('Zamknij temat'), required=False,
...
class TopicForm(PostForm):
    ...
    topic_type = forms.ChoiceField(
        label=_('Typ tematu'), choices=Topic.TYPE_CHOICES, required=False)
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.can_create_polls:
            self.fields['poll_question'] = forms.CharField(
                label=_('Pytanie ankiety'), required=False,
                help_text=_(
                    'Wpisz pytanie aby dołączyć ankietę lub pozostaw pole puste.'
                    'create a poll.'
                ),
                max_length=TopicPoll._meta.get_field('question').max_length,
            )
            self.fields['poll_max_options'] = forms.IntegerField(
                label=_('Maksymalna liczba opcji ankiety dla użytkownika'), required=False,
                help_text=_('Tyle opcji może wybrać użytkownik podczas odpowiadania'),
            ...
            self.fields['poll_duration'] = forms.IntegerField(
                label=_('Przez ile dni ankieta ma być aktywna?'), required=False,
                help_text=_('Wpisz 0 lub pozostaw puste dla niekończącej się ankiety.'),
                min_value=0, initial=0,
            )
            self.fields['poll_user_changes'] = forms.BooleanField(
                label=_('Pozwalaj na zmianę odpowiedzi?'), required=False,
                help_text=_('Określa czy użytkownik może zmienić odpowiedź.'),
                initial=False,
            )
    ...
    def clean(self):
        ...
        if self.cleaned_data.get('poll_question', None) \
            ...
            self.add_error(
                'poll_max_options',
                _('Musisz ustalić maksymalną liczbę odpowiedzi na użytkownika'),
            )
