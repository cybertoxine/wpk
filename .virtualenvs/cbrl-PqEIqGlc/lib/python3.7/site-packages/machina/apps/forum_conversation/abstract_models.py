...
class AbstractTopic(DatedModel):
    ...
    subject = models.CharField(max_length=255, verbose_name=_('Tytuł'))
    ...
    TYPE_CHOICES = (
        (TOPIC_POST, _('Domyślny')),
        (TOPIC_STICKY, _('Przyklejony')),
        (TOPIC_ANNOUNCE, _('Ogłoszenie')),
    )
    type = models.PositiveSmallIntegerField(
        choices=TYPE_CHOICES, db_index=True, verbose_name=_('Typ tematu'),
    )

    # A topic can be locked, unlocked or moved.
    TOPIC_UNLOCKED, TOPIC_LOCKED, TOPIC_MOVED = 0, 1, 2
    STATUS_CHOICES = (
        (TOPIC_UNLOCKED, _('Temat otwarty')),
        (TOPIC_LOCKED, _('Temat zamknięty')),
        (TOPIC_MOVED, _('Temat przeniesiony')),
    )
    status = models.PositiveIntegerField(
        choices=STATUS_CHOICES, db_index=True, verbose_name=_('Status tematu'),
    )
    ...
    approved = models.BooleanField(default=True, db_index=True, verbose_name=_('Zatwierdzony'))

    # The number of posts included in this topic (only those that are approved).
    posts_count = models.PositiveIntegerField(
        editable=False, blank=True, default=0, verbose_name=_('Liczba postów'),
    )

    # The number of time the topic has been viewed.
    views_count = models.PositiveIntegerField(
        editable=False, blank=True, default=0, verbose_name=_('Liczba wyświetleń'),
    )

    # The date of the latest post.
    last_post_on = models.DateTimeField(blank=True, null=True, verbose_name=_('Ostatni post dodano'))
    ...
    first_post = models.ForeignKey(
        ...
        on_delete=models.SET_NULL, verbose_name=_('Pierwszy post'),
    )
    last_post = models.ForeignKey(
        'forum_conversation.Post', editable=False, related_name='+', blank=True, null=True,
        on_delete=models.SET_NULL, verbose_name=_('Ostatni post'),
    )

    # Many users can subscribe to this topic
    subscribers = models.ManyToManyField(
        settings.AUTH_USER_MODEL, related_name='topic_subscriptions', blank=True,
        verbose_name=_('Subskrybenci'),
    )
    ...
    def clean(self):
        ...
        if self.forum.is_category or self.forum.is_link:
            raise ValidationError(
                _('Temat nie może być powiązany z kategorią lub odnośnikiem do forum')
            )
...
class AbstractPost(DatedModel):
    ...
    anonymous_key = models.CharField(
        max_length=100, blank=True, null=True, verbose_name=_('Identyfikator anonimowego użytkownika'),
    )

    # Each post can have its own subject. The subject of the thread corresponds to the
    # one associated with the first post
    subject = models.CharField(verbose_name=_('Tytuł'), max_length=255)
    ...
    content = MarkupTextField(
        ...
        verbose_name=_('Treść'),
    )

    # Username: if the user creating a topic post is not authenticated, he must enter a username
    username = models.CharField(max_length=155, blank=True, null=True, verbose_name=_('Nazwa użytkownika'))
    ...
    enable_signature = models.BooleanField(
        default=True, db_index=True, verbose_name=_('Dołącz podpis'),
    )
    ...
    update_reason = models.CharField(
        max_length=255, blank=True, null=True, verbose_name=_('Powód edycji'),
    )

    # Tracking data
    updated_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, editable=False, blank=True, null=True, on_delete=models.SET_NULL,
        verbose_name=_('Ostatnio edytowany przez'),
    )
    updates_count = models.PositiveIntegerField(
        editable=False, blank=True, default=0, verbose_name=_('Licznik edycji'),
    )
    ...
    def clean(self):
        ...
        if self.poster is None and self.anonymous_key is None:
            raise ValidationError(
                _('ID użytkownika lub identyfikator anonimowego uzytkownika musi być dołączony do postu.'),
            )
        if self.poster and self.anonymous_key:
            raise ValidationError(
                _('ID użytkownika lub identyfikator anonimowego uzytkownika musi być dołączony do postu, nie oba.'),
            )

        if self.anonymous_key and not self.username:
            raise ValidationError(_('Nazwa użytkownika musi być podana jeżeli autor jest anonimowy'))
