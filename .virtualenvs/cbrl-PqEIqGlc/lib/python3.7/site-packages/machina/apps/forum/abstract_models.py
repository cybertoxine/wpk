...
class AbstractForum(MPTTModel, DatedModel):
    ...
    parent = TreeForeignKey(
        ...
        verbose_name=_('Rodzic'),
    )

    name = models.CharField(max_length=100, verbose_name=_('Nazwa'))
    slug = models.SlugField(max_length=255, verbose_name=_('Slug'))

    description = MarkupTextField(verbose_name=_('Opis'), null=True, blank=True)
    ...
    image = ExtendedImageField(
        ...
        verbose_name=_('Logo forum'), **machina_settings.DEFAULT_FORUM_IMAGE_SETTINGS
    )

    # Forums can be simple links (eg. wiki, documentation, etc)
    link = models.URLField(verbose_name=_('Odnośnik forum'), null=True, blank=True)
    link_redirects = models.BooleanField(
        default=False, verbose_name=_('Licznik przekierowań'),
        help_text=_('Zliczaj liczbę kliknięć w odnośnik do forum'),
    )

    # Category, Default forum or Link ; that's what a forum can be
    FORUM_POST, FORUM_CAT, FORUM_LINK = 0, 1, 2
    TYPE_CHOICES = (
        (FORUM_POST, _('Domyślne forum')),
        (FORUM_CAT, _('Kategoria')),
        (FORUM_LINK, _('Odnośnik')),
    )
    type = models.PositiveSmallIntegerField(
        choices=TYPE_CHOICES, verbose_name=_('Typ forum'), db_index=True,
    )
    ...
    direct_topics_count = models.PositiveIntegerField(
        editable=False, blank=True, default=0, verbose_name=_('Liczba tematów'),)
    link_redirects_count = models.PositiveIntegerField(
        editable=False, blank=True, default=0, verbose_name=_('Licznik przekierowań'),
    )
    ...
    last_post_on = models.DateTimeField(verbose_name=_('Ostatni post dodano'), blank=True, null=True)
    ...
    display_sub_forum_list = models.BooleanField(
        default=True, verbose_name=_('Wyświetlaj w liście rodzica'),
        help_text=_('Wyświetlaj forum w liście forów rodzica.'),
    )
    ...
    def clean(self):
        ...
        if self.parent and self.parent.is_link:
            raise ValidationError(_('Forum nie może mieć odnośnika do forum jako rodzica'))

        if self.is_category and self.parent and self.parent.is_category:
            raise ValidationError(_('Kategoria nie może mieć innej kategorii jako rodzica'))

        if self.is_link and not self.link:
            raise ValidationError(_('Odnośnik do formu musi mieć przypisany odnośnik'))
