...
class ForumAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        [_('Ustawienia forum'), {'fields': ('display_sub_forum_list', ), 'classes': ('collapse', )}],
        [
            _('Ustawienia odnośnika do forum'),
    ...
    def moveforum_view(self, request, forum_id, direction):
        ...
        try:
            ...
            pass
        self.message_user(request, _("Pomyślnie przeniesiono forum '{}'").format(forum.name))
    ...
    def editpermissions_index_view(self, request, forum_id=None):
        ...
        context['forum'] = forum
        context['title'] = _('Uprawnienia forum') if forum else _('Globalne uprawnienia forum')
        ...
        if forum and request.method == 'POST':
            ...
            if forum_form.is_valid() and forum_form.cleaned_data['forum']:
                ...
                self.message_user(request, _('Uprawnienia skopiowano pomyślnie'))
            ...
            if user_form.is_valid() and group_form.is_valid():
                ...
                if not user and not anonymous_user and not group:
                    user_form._errors[NON_FIELD_ERRORS] = user_form.error_class(
                        [_('Wybierz ID użytkownika, ID grupy lub anonimowego użytkownika'), ])
                elif user:
                    # Redirect to user
                    url_kwargs = (
    ...
    def editpermissions_user_view(self, request, user_id, forum_id=None):
        ...
        context = self.get_forum_perms_base_context(request, forum)
        context['forum'] = forum
        context['title'] = '{} - {}'.format(_('Uprawnienia forum'), user)
    ...
    def editpermissions_anonymous_user_view(self, request, forum_id=None):
        ...
        context = self.get_forum_perms_base_context(request, forum)
        context['forum'] = forum
        context['title'] = '{} - {}'.format(_('Uprawnienia forum'), _('Anonimowy użytkownik'))
        context['form'] = self._get_permissions_form(
    ...
    def editpermissions_group_view(self, request, group_id, forum_id=None):
        ...
        context = self.get_forum_perms_base_context(request, forum)
        context['forum'] = forum
        context['title'] = '{} - {}'.format(_('Uprawnienia forum'), group)
        context['form'] = self._get_permissions_form(
    ...
    def _get_permissions_form(self, request, permission_model, filter_kwargs):
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                self.message_user(request, _('Nadano uprawnienia'))
        else:
