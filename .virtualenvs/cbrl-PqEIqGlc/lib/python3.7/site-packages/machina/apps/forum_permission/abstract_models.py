...
class AbstractForumPermission(models.Model):
    ...
    codename = models.CharField(
        max_length=150, verbose_name=_('Kod uprawnienia'), unique=True, db_index=True,
    )

    class Meta:
        abstract = True
        app_label = 'forum_permission'
        verbose_name = _('Uprawnienia forum')
        verbose_name_plural = _('Uprawnienia forum')
...
class BaseAuthForumPermission(models.Model):
    ...
    has_perm = models.BooleanField(verbose_name=_('Ma uprawnienie'), default=True, db_index=True)
...
class AbstractUserForumPermission(BaseAuthForumPermission):
    ...
    anonymous_user = models.BooleanField(
        verbose_name=_('Nadaj użytkownikowi anonimowemu'), default=False, db_index=True,
    )
    ...
    class Meta:
        ...
        app_label = 'forum_permission'
        verbose_name = _('Uprawnienie użytkownika forum')
        verbose_name_plural = _('Uprawnienia użytkownika forum')
    ...
    def clean(self):
        ...
        if (
            ...
            raise ValidationError(
                _('Uprawnienie musi wskazywać użytkownika lub użytkownika anonimowego.'),
            )
...
class AbstractGroupForumPermission(BaseAuthForumPermission):
    ...
    group = models.ForeignKey(Group, on_delete=models.CASCADE, verbose_name=_('Grupa'))

    class Meta:
        abstract = True
        unique_together = ('permission', 'forum', 'group', )
        app_label = 'forum_permission'
        verbose_name = _('Uprawnienie grupy')
        verbose_name_plural = _('Uprawnienia grupy')
